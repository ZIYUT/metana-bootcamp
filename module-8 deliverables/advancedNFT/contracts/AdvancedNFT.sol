...
contract AdvancedNFT is ERC721, Multicall, Ownable {
    ...
    address public addressForWithdraw;
    ...
    constructor(bytes32 _merkleRoot, address _addressForWithdraw) ERC721("AdvancedNFT", "ANFT") Ownable(msg.sender) {
    ...
    // Update the sale states
    function setSaleState(SaleState _state) external onlyOwner {
        saleState = _state;
        emit StateChanged(_state);
    }

    // Commit and reveal phase for random NFT ID allocation
    ...
    function withdrawFunds(address[] calldata _contributors) external {
        require(msg.sender == addressForWithdraw, "Not the withdraw address");
